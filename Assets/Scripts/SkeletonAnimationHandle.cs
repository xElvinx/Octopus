...
public class SkeletonAnimationHandle : MonoBehaviour
{
	...
	private SkeletonAnimation _currentSkeletonAnimationHorizontal;
	...
	void Awake () {
		...
		foreach (var entry in statesAndAnimations) {
			...
			entry.skeletonAnimation.gameObject.SetActive(false);
		}
	...
	public void SetFlip (float horizontal) {
		if (horizontal != 0) {
			_currentSkeletonAnimationHorizontal.Skeleton.ScaleX = horizontal > 0 ? 1f : -1f;
		}
	}
	...
	public Spine.Animation GetAnimationForState (int shortNameHash) {
		...
		if (_currentSkeletonAnimationHorizontal) _currentSkeletonAnimationHorizontal.gameObject.SetActive(false);
		_currentSkeletonAnimationHorizontal = foundState.skeletonAnimation;
		_currentSkeletonAnimationHorizontal.gameObject.SetActive(true);
	...
	public void PlayNewAnimation (Spine.Animation target, int layerIndex) {
		...
		if (transition != null) {
			_currentSkeletonAnimationHorizontal.AnimationState.SetAnimation(layerIndex, transition, false);
			_currentSkeletonAnimationHorizontal.AnimationState.AddAnimation(layerIndex, target, true, 0f);
		} else {
			_currentSkeletonAnimationHorizontal.AnimationState.SetAnimation(layerIndex, target, true);
		}
	...
	public void PlayOneShot (Spine.Animation oneShot, int layerIndex) {
		var state = _currentSkeletonAnimationHorizontal.AnimationState;
	...
	Spine.Animation GetCurrentAnimation (int layerIndex) {
		var currentTrackEntry = _currentSkeletonAnimationHorizontal.AnimationState.GetCurrent(layerIndex);
